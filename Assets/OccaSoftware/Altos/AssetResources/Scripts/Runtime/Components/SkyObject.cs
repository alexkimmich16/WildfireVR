...
namespace OccaSoftware.Altos.Runtime
{
    ...
    public class SkyObject : MonoBehaviour
    {
        ...
        public static void CheckCount()
		{
            if(_Sun.Count <= 0)
			{
                Debug.LogWarning("Altos: No SkyObjects are marked as type Sun. As a result, your atmosphere will not account for sun atmospheric lighting. Mark a SkyObject as type Sun to ensure the atmospheric lighting will render correctly.");
			}
            if(_Sun.Count > 1)
			{
                Debug.LogWarning("Altos: More than one SkyObject is marked as type Sun. This could lead to undefined behavior. Ensure that only one SkyObject in a scene is marked as type Sun.");
			}
		}
        */
        ...
        public Material GetMaterial()
		{
            if (material == null)
            {
                Shader s = FindObjectOfType<AltosSkyDirector>()?.altosData.shaders.skyObjectShader;
                if (s != null)
                {
