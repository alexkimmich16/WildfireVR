...
void CheckerboardUpsample_half(Texture2D Tex, half2 UV, out half3 Color, out half Alpha)
{
	...
	for (int f = 0; f < 4; f++)
	{
		float sDepthRaw = _DitheredDepthTexture.SampleLevel(point_clamp_sampler, (UV + o[f] * texCoord), 0).r;
		float sDepth01 = Linear01Depth(sDepthRaw, _ZBufferParams);
		
		
		if (depth01 - sDepth01 < 0.001)
		{
	...
	int rMod = floor(rand2dTo1d(UV + _Time.yy) * validCounter);
	col = colors[rMod];
