...
namespace Menu
{
    public class MenuEffect : SerializedMonoBehaviour
    {
        ...
        [FoldoutGroup("ObjectRef")] public Transform Cam;
        ...
        private void Update()
        {
            ...
            Vector3 ForwardRot = Quaternion.Euler(new Vector3(0, Cam.eulerAngles.y, 0)) * Vector3.forward;
            transform.position = Vector3.Lerp(transform.position, Cam.position + (ForwardRot * MenuDistance), Time.deltaTime * positionSpeed);
            transform.rotation = Quaternion.Lerp(transform.rotation, Quaternion.Euler(new Vector3(0, Cam.eulerAngles.y, 0)), Time.deltaTime * rotationSpeed);
        }
