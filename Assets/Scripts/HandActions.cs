...
public class HandActions : MonoBehaviour
{
    ...
    public int PastFrameCount = 100;

    public int AverageVelCheckIndex;
    //[HideInInspector]
   ...
   // public Vector3 MyEuler;
    ...
    public float Average(List<float> list)
    {
        ...
        for (int i = 0; i < list.Count; i++)
            average += list[i];
        float Average = average / list.Count;
        //AverageVel = Average;
        return Average;
    }
    ...
    void Update()
    {
        ...
        //AverageVel = AverageVelocity(AverageVelCheckIndex, AverageVelCheckIndex);
        LastFrameSave();
    ...
    public Vector3 StartEndDirection(int Min, int Max)
    {
        //Min = AverageVelCheckIndex;
        //Max = AverageVelCheckIndex;
        /*
        ...
        for (int i = 0; i < Max + 1; i++)
            if(i == Min )
                Total += 
            if(i == Max)
                Total += PastFrames[PastFrames.Count - i - 1];
        */
        
        return PastFrames[PastFrames.Count - Min - 1] - PastFrames[PastFrames.Count - Max - 1];
    }
    public void LastFrameSave()
    {
        PastFrames.Add(transform.position);
    ...
    */
    public void SetRemoteStats()
    {
        ...
        //device.TryGetFeatureValue(CommonUsages., out Velocity);
